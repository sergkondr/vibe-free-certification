...
[![Test](https://github.com/skondrashov/vibe-free-certification/actions/workflows/test.yml/badge.svg)](https://github.com/skondrashov/vibe-free-certification/actions/workflows/test.yml)
...
jobs:
  certify:
    ...
    timeout-minutes: 2
    steps:
      - uses: skondrashov/vibe-free-certification@v1
        with:
...
The recommended workflow grants only `issues: write`. GitHub does not offer permission scoped to one pull request, so the action can modify labels on other issues and pull requests while it runs. Do not pass a personal access token or additional secrets; use the job's short-lived `${{ github.token }}`.
